...
The Equinox API uses a **challenge-response authentication flow** that leverages Stellar Ed25519 key pairs to cryptographically verify wallet ownership. This replaces the insecure `x-user-address` header pattern that previously allowed trivial impersonation.
