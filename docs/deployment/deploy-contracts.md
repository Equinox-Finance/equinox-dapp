...
This guide covers the complete deployment of the Equinox smart contracts to Stellar/Soroban networks. Read it fully before executing any command - the order of steps is mandatory.
...
Equinox deploys a **single WASM binary** that contains both the property tokenization and DeFi lending logic. There are not two separate contracts - there is one contract, one contract ID, one deployment.
...
```bash
cd equinox-defi-rwa/apps/contracts
...
Update `equinox-defi-rwa/apps/api/.env`:
