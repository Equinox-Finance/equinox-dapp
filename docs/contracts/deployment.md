...
All four Equinox Land game contracts were built with `stellar contract build` (target `wasm32v1-none`) and deployed to Stellar testnet. Deployment order: PropertyNFT and LandToken first (no inter-dependencies), then GameMarketplace and GameEngine (both depend on the first two).
...
stellar contract invoke --id CCPUVGQAMDUUASHMXB7Z6F6XHCZI2WXOPR7DXEVPJBEGYZVJEABEABLE \
  --source equinox-game-deployer --network testnet \
  -- initialize \
...
stellar contract invoke --id CCPUVGQAMDUUASHMXB7Z6F6XHCZI2WXOPR7DXEVPJBEGYZVJEABEABLE \
  --source equinox-game-deployer --network testnet --send=no \
...
stellar keys generate equinox-deployer --network testnet --fund

# 3. Deploy two instances (constructor takes the admin address)
ADMIN=$(stellar keys address equinox-deployer)
WASM=../../target/wasm32v1-none/release/rwa_defi_contract.wasm
stellar contract deploy --wasm "$WASM" --source equinox-deployer --network testnet -- --admin "$ADMIN"
stellar contract deploy --wasm "$WASM" --source equinox-deployer --network testnet -- --admin "$ADMIN"
