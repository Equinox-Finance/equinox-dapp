# Equinox Land: Developer Setup Guide

This guide walks you through setting up Equinox Land for local development. All commands are copy-pasteable and tested.
...
psql -U postgres -c "CREATE DATABASE equinox_dev;"
psql -U postgres -c "CREATE DATABASE equinox_test;"
```
...
DATABASE_URL=postgres://postgres:postgres@localhost:5432/equinox_dev
...
docker run -d \
  --name equinox-pg-test \
  -e POSTGRES_USER=test \
  -e POSTGRES_PASSWORD=test \
  -e POSTGRES_DB=equinox_test \
  -p 5432:5432 \
  postgres:16

# Set test database URL
export DATABASE_URL=postgres://test:test@localhost:5432/equinox_test
...
docker stop equinox-pg-test && docker rm equinox-pg-test
```
...
### "Cannot find module '@equinox/shared'"
...
psql -U postgres -c "\l" | grep equinox_dev
