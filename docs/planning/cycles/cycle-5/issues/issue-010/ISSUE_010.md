...
Build `src/components/game/PropertyPanel.tsx` inside `apps/equinox-land`. On desktop, the panel slides in from the right as a fixed-width sidebar when a tile is selected. On mobile, it rises as a bottom sheet with a drag handle.
