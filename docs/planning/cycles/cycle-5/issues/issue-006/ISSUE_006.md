...
The GameEngine is the core rules enforcer for Equinox Land. It handles property improvements (upgrading buildings from Vacant to Residential to Commercial to Skyscraper), rental income calculation, and the claim flow. It is also the only contract authorized to update improvement levels and last-claimed-ledger values on the PropertyNFT contract.
