...
Build `src/app/marketplace/page.tsx` inside `apps/equinox-land`. The page shows a grid of active listings. Each listing card shows a large color-coded tile preview, the property coordinates, the building level with its income rate, the asking price in LAND, the seller's abbreviated address, and the time since the listing was created.
