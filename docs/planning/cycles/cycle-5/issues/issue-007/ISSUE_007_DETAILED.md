# C5-007: Bootstrap equinox-land Next.js App and Design System
...
```bash
bunx create-next-app@latest equinox-land \
  --typescript \
  --tailwind \
  --app \
  --src-dir \
...
```json
{
  "compilerOptions": {
    ...
    "paths": {
      "@/*": ["./src/*"],
      "@equinox/shared": ["../shared/src/index.ts"]
    }
  }
}
```
...
- `apps/equinox-land/` is initialized and starts on port 3001 with `bun run dev`.
