# Bootstrap equinox-land Next.js App and Design System

## Context

Equinox Land is a standalone product that lives completely apart from the main Equinox investment platform. It has its own identity, its own users (game players, not necessarily investors), and its own technical stack. Building it as a separate Next.js application at `apps/equinox-land/` reflects this separation clearly and avoids coupling two products that should evolve independently.
...
Create `apps/equinox-land/` as a new Next.js 15 application using the App Router. Initialize it with its own `package.json` (separate from `apps/webapp`), its own `tailwind.config.ts`, and its own `next.config.ts`. Configure TypeScript with `paths` aliasing so `@/` resolves to `src/` and `@equinox/shared` resolves to the shared package.

Define the game's design system in `tailwind.config.ts`: a dark-first color palette distinct from the main platform, a custom font stack, and utility tokens for the game's tile grid. The game brand should feel like a product in its own right, not a section of equinox.com.
...
- `apps/equinox-land/` exists with its own `package.json`, separate from any other app.
- `bun run dev` inside `apps/equinox-land/` starts the app on its own port.
