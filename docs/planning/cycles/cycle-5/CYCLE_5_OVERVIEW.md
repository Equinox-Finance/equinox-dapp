# Cycle 5: Equinox Land: Real Estate Game on Stellar
...
```
apps/
  equinox-land/                    ← standalone Next.js app (this cycle)
...
| C5-007 | Bootstrap equinox-land Next.js app and design system      | GAME      | Medium     | None                           |
...
| ------------------------- | -------------------------------------------------------------------------------------- |
| App deploys independently | equinox-land deploys to Vercel without any dependency on apps/api or apps/webapp        |
...
- `apps/equinox-land` must be completley self-contained. No imports from `apps/api` or `apps/webapp`.
...
- The event indexer (C5-014) lives inside equinox-land as a Next.js Route Handler, not in apps/api.
