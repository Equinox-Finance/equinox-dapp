...
This guide provides step-by-step instructions to set up the Equinox development environment on your local machine.
...
- `DATABASE_URL`: Connection string for PostgreSQL (default: `postgresql://user:password@localhost:5432/equinox_defi`).
...
**Run Equinox Land (Game):**

```bash
cd apps/equinox-land && bun run dev
```
